...
/**
 * Structural template for the Elastic `filestream` integration package at the older package
 * version 1.1.3, registered alongside `filestreamPackageTemplate_1_1_5` (1.1.5) so policies still
 * pinned to 1.1.3 get a structured editor too. Derived from the published package registry
 * snapshot: https://epr.elastic.co/epr/filestream/filestream-1.1.3.zip (manifest.yml +
 * data_stream/generic/manifest.yml). Diffed byte-for-byte against the 1.1.5 sibling's
 * manifests: the only differences are the `version` field and cosmetic markdown formatting
 * in var descriptions (e.g. `**bold**` vs `** bold **`) — every var name, type, default,
 * and required flag is identical, so nothing structurally differs here beyond the version
 * string.
 */
export const filestreamPackageTemplate_1_1_3: PackageTemplate = {
  name: 'filestream',
