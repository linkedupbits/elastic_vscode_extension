...
/**
 * Structural template for the Elastic `apache` integration package (package version 2.0.0),
 * derived from the published package registry snapshot:
 * https://epr.elastic.co/epr/apache/apache-2.0.0.zip
 ...
 * Drives both the default values used when creating a new Apache integration policy, and
 * the structured editor form.
 * No data stream in this manifest declares `agent.privileges.root`, so no stream here sets
 * `requiresRoot` (mirrors Nginx, unlike System's auth/syslog/diskio).
 * `APACHE_METRICS_SSL_DEFAULT` is verbatim from the manifest's commented-out example
 * `certificate_authorities` block, trailing whitespace on its last line included.
 */
export const apachePackageTemplate_2_0_0: PackageTemplate = {
  name: 'apache',
