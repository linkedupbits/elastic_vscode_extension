 ...
 *
 * IMPORTANT PROVENANCE NOTE: version `9.4.3` was never published to the Elastic Package
 * Registry. `curl https://epr.elastic.co/package/apm/9.4.3/` returns 404 ("artifact not
 * found"), and `https://epr.elastic.co/search?package=apm&all=true&prerelease=true` shows the
 * real version sequence on the 9.x line is only ever
 * 9.0.0-preview-1738343125 -> 9.1.0-preview-1744129488 -> 9.1.0-preview-1747764883 ->
 * 9.3.4-preview-1775552749 -> 9.3.5-preview-1779430085, with no 9.2.x or 9.4.x build ever
 * released (mirrors `apmPackageTemplate_9_0_3`'s own gap, one version line up). All field
 * content below is therefore sourced from the nearest real EPR build below 9.4.3,
 * `apm-9.3.5-preview-1779430085`: https://epr.elastic.co/epr/apm/apm-9.3.5-preview-1779430085.zip
 * (manifest.yml). The `version` field is kept as `9.4.3` to match the package identity this
 * template was requested under.
 *
 ...
 * there is no data_stream directory in the 9.3.5-preview build (or any 9.x build) to read
 ...
 *
 * New since `apmPackageTemplate_9_0_3` (sourced from 9.0.0-preview): two tail-sampling vars
 * were added, `tail_sampling_ttl` (text, default `"30m"`, inserted between
 * `tail_sampling_interval` and `tail_sampling_enabled`) and
 * `tail_sampling_discard_on_write_failure` (bool, default `false`, appended after
 * `tail_sampling_storage_limit`) - every other var, default and ordering is unchanged.
 */
export const apmPackageTemplate_9_4_3: PackageTemplate = {
  name: 'apm',
  title: 'Elastic APM',
  version: '9.4.3',
  inputs: [
    {
      ...
      vars: [
        ...
        { key: 'tail_sampling_ttl', label: 'Tail Sampling TTL', type: 'string', default: '30m' },
        ...
        { key: 'tail_sampling_storage_limit', label: 'Tail Sampling Storage Limit', type: 'string', default: '0GB' },
        {
          ...
          label: 'Tail Sampling Discard On Write Failure',
          type: 'boolean',
          default: false,
        },
      ],
