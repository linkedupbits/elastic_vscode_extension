...
| `system` | System | 2.21.0 | `systemPackage_2_21_0.ts` | `epr.elastic.co/epr/system/system-2.21.0.zip` | verified byte-for-byte structurally identical to 2.22.1 (manifest.yml and every data stream manifest differ only in the `version:` field) |
...
| `nginx` | Nginx | 2.3.2 | `nginxPackage_2_3_2.ts` | `epr.elastic.co/epr/nginx/nginx-2.3.2.zip` | verified byte-for-byte structurally identical to 2.0.0 (manifests differ only in the `version:`/`format_version:` fields) - like 2.0.0, and unlike 3.2.1, neither input declares the `condition` var |
...
| `apache` | Apache HTTP Server | 2.1.1 | `apachePackage_2_1_1.ts` | `epr.elastic.co/epr/apache/apache-2.1.1.zip` | verified structurally identical to 2.0.0 (same inputs/streams/vars/defaults/required flags); only cosmetic manifest differences (trailing whitespace on the SSL default's last line, a description line-wrap) |
...
| `apm` | Elastic APM | 9.4.3 | `apmPackage_9_4_3.ts` | `epr.elastic.co/epr/apm/apm-9.3.5-preview-1779430085.zip` ⚠️ | **version 9.4.3 was never published to EPR** (the real 9.x sequence is only 9.0.0-preview → 9.1.0-preview ×2 → 9.3.4-preview → 9.3.5-preview, with no 9.2.x or 9.4.x build); field data was sourced from the nearest real build below 9.4.3, 9.3.5-preview, while the `version` string was kept as `9.4.3`. Same single-input/placeholder-stream shape as 9.0.3 (see below); new since 9.0.3: two tail-sampling vars, `tail_sampling_ttl` and `tail_sampling_discard_on_write_failure` |
...
| `postgresql` | PostgreSQL | 1.29.0 | `postgresqlPackage_1_29_0.ts` | `epr.elastic.co/epr/postgresql/postgresql-1.29.0.zip` | same shape as 1.28.0; new since 1.28.0: the `log` stream gained an optional `tz_map` var (yaml, for mapping timezone abbreviations found in PostgreSQL's own log timestamps to IANA zone names) |
...
| `docker` | Docker | 2.15.2 | `dockerPackage_2_15_2.ts` | `epr.elastic.co/epr/docker/docker-2.15.2.zip` | inputs: `docker/metrics` (streams `container`, `cpu`, `diskio`, `event`, `healthcheck`, `image` - disabled by default, `info`, `memory`, `network`; carries the input-level `podman` boolean var), `filestream` (stream `container_logs`, no input-level vars); no `requires_root` |
| `synthetics` | Elastic Synthetics | 1.7.0 | `syntheticsPackage_1_7_0.ts` | `epr.elastic.co/epr/synthetics/synthetics-1.7.0.zip` | shaped like APM, not nginx/system: four inputs (`synthetics/http`, `synthetics/tcp`, `synthetics/icmp`, `synthetics/browser`, none with input-level vars), each with one "config" stream of 20-41 monitor-config vars (schedule, target, auth, SSL, retry/maintenance-window behaviour); `synthetics/browser` additionally has two vars-less streams (`browser.network`, `browser.screenshot`). Every config stream sets manifest `enabled: false` (Synthetics monitors are opt-in), so every stream here defaults to disabled while the owning inputs default enabled; no `requires_root` |
...
* **Fleet Agent Policies**, which itself expands into one node per Kibana Space before showing any policies (`getLiveAgentPolicySpaceItems` in `src/treeView/elasticTreeProvider.ts`, reusing the exact same `fetchSpaces` call as the Spaces node above, rendered as `connection-agentpolicies-space` tree items carrying the fetched `SpaceDefinition` as `ElasticTreeItem.liveSpace`). Expanding one of those space nodes fetches that space's Fleet Agent Policies via the [Get Agent Policies API](https://www.elastic.co/docs/api/doc/kibana/operation/operation-get-fleet-agent-policies) (`connection-agentpolicies-space` → `connection-agentpolicy` children, following the API's pagination in 100-item pages via `fetchAllPages` in `src/connections/kibanaClient.ts`, so a space with more than 100 agent policies is still shown in full). The request is scoped to that space using Kibana's general space-aware URL convention - `/s/<space_id>` prefixed onto the path (`spaceScopedPath` in `src/connections/kibanaClient.ts`) - except the "default" space, which is left unprefixed, identical to how this endpoint was called before spaces were introduced into this part of the tree. Agent policies reuse the same `FleetAgentPolicy` shape as the local `Fleet_Agent_Policies/` artifact type (see above). Because Fleet lets one agent policy belong to more than one space, the same policy `id` can legitimately appear more than once in this tree (once per space it's assigned to) - `ElasticTreeItem.liveSpaceId` records which space a given `connection-agentpolicy`/`connection-integrationpolicy` row was fetched under, so a row's true identity in the tree is the pair `(liveSpaceId, liveAgentPolicy.id)`, not the bare policy id.

**Live Integration Policies.** Expanding an agent-policy space node also fetches that deployment's Integration (package) Policies live via the [Get Package Policies API](https://www.elastic.co/docs/api/doc/kibana/operation/operation-get-fleet-package-policies) (`fetchPackagePolicies` in `src/connections/kibanaClient.ts`, also paginated via `fetchAllPages`, scoped to the same space as the agent policies fetch alongside it) - concurrently with the agent policies fetch (`Promise.all` in `getLiveAgentPolicyItems`, `src/treeView/elasticTreeProvider.ts`), so neither request blocks the other and the (usually larger) integration policy list never delays the agent policy list from appearing. Since both lists always come from one single space-scoped pair of requests, matching them needs no space-aware id lookup of its own - the scoping is already done by the fetch: each fetched integration policy is assigned client-side to every agent policy it belongs to within that space, matching on `policy_id` or membership in `policy_ids` - the same fields Fleet itself uses - and the matches are stashed directly on that agent policy's tree item (`ElasticTreeItem.liveIntegrationPolicies`), so expanding an individual agent policy needs no further API call. An agent policy with at least one assigned integration policy becomes expandable (`connection-agentpolicy` → `connection-integrationpolicy` children); one with none stays a leaf. If the integration policies fetch itself fails (e.g. the API key lacks that permission), agent policies still show, just without any children - the failure is swallowed rather than surfaced, since it shouldn't block the (more important) agent policy list.

Clicking a live integration policy opens `LiveIntegrationPolicyViewPanel` (`src/editors/liveIntegrationPolicyViewPanel.ts`), a read-only webview that deliberately reuses the exact same structured input/stream/var rendering `IntegrationPolicyEditorPanel` uses, rather than a hand-written summary, so a live policy looks and lays out identically to a downloaded one being edited - just disabled, with no Save button. This is only possible because the rendering logic was extracted out of `media/integrationPolicyForm.js` into a shared client-side module, `media/integrationPolicyRender.js` (`window.IntegrationPolicyRender`): `renderTemplate(container, template, readonly)` builds the same `<details>`/checkbox/input markup either way, but with every control `readonly`/`disabled` when `readonly` is true, and `populateValues(template, item)` sets each control's value regardless of mode. `integrationPolicyForm.js` (the editable form) now calls into this shared module instead of duplicating it, and `ArtifactPanelBase` (`src/editors/artifactPanelBase.ts`) was extended to load more than one script file in order (`scriptFileNames: string | string[]`) so `IntegrationPolicyEditorPanel` can load `integrationPolicyRender.js` before `integrationPolicyForm.js`. `LiveIntegrationPolicyViewPanel` isn't file-backed and has no save handshake, so it doesn't extend `ArtifactPanelBase` - instead it builds its own minimal HTML shell (readonly Connection/Integration Type/Namespace/Description/Agent Policy display fields, an `#inputs-container`, and the JSON-fallback textarea for when no `PackageTemplate` is registered for the package/version, mirroring `IntegrationPolicyEditorPanel`'s own fallback) and loads `integrationPolicyRender.js` followed by its own small view script, `media/liveIntegrationPolicyView.js`, which calls `renderTemplate(..., true)`/`populateValues(...)` against a payload embedded directly in the page (`window.__liveIntegrationPolicy`, JSON-serialized server-side with `<` escaped to `<` so a value containing `</script>` can't prematurely close the embedding script tag) rather than the `ready`/`init` postMessage handshake `ArtifactPanelBase` uses, since the data is already known synchronously when the panel opens. `getNonce()` (CSP nonce generation) was pulled out to `src/editors/webviewNonce.ts` so both `ArtifactPanelBase` and this panel share one implementation. Unlike every other live/read-only view in this project (`LiveAgentPolicyViewPanel`/`LiveSpaceViewPanel`, both `enableScripts: false` with server-rendered HTML), this one needs `enableScripts: true` specifically to run the shared renderer client-side - that's the tradeoff for looking identical to the editable form instead of a bespoke read-only layout.

Each live Space/Fleet Agent Policy/Integration Policy row also has an inline **Download to Project** action (`src/repositories.ts`'s `downloadSpace`/`downloadAgentPolicy`/`downloadIntegrationPolicy`, wired up in `src/extension.ts`) - the one place this project's usual direction (local file → applied to a deployment) runs in reverse. It saves the live item as a normal local artifact under `Spaces/`/`Fleet_Agent_Policies/`/`Fleet_Agent_Policies/<name>/Integrations/`, using the exact same `saveSpace`/`saveFleetAgentPolicy`/`saveIntegrationPolicy` functions and file-naming conventions the structured editors use, after stripping the live API response down to the known fields (a real Kibana response can carry extra fields - e.g. a built-in Space's `_reserved` flag, or an agent policy's `status`/`revision`/`package_policies` - that aren't part of this project's curated shape). If a local artifact with that id/name already exists, the user is prompted to confirm before it's overwritten (mirroring the confirmation modal `elasticSource.deleteArtifact` already uses). A downloaded Fleet Agent Policy's `name` is validated the same way the local "New Fleet Agent Policy" form validates one (`validateArtifactName`), since unlike a Kibana space id, Kibana doesn't constrain a policy name to filesystem-safe characters.

Downloading a live Integration Policy specifically requires its *owning* Fleet Agent Policy to already have been downloaded to the project: `findAgentPolicyFilePathById` (`src/repositories.ts`) looks for a local Agent Policy whose own downloaded `id` field (preserved as-is by `downloadAgentPolicy`) matches the live agent policy the integration policy's tree row is nested under - not merely the integration policy's own `policy_id`, since a single integration policy can be shared across several agent policies via `policy_ids` and appear as a child under each of them. If no local match is found, the download is refused with a warning telling the user to download the owning Agent Policy first, rather than silently auto-creating one or guessing which agent policy folder to use.

**Known gap:** unlike the live tree above, this lookup is *not* space-aware - it matches on bare agent-policy `id` across every locally downloaded policy regardless of which space either side came from. See [`extension_todo.md`](extension_todo.md) for the details and what fixing it would require.

Before that name validation runs, a downloaded Fleet Agent Policy's (or Integration Policy's) live name is first passed through `extractDownloadedArtifactName` (`src/repositories.ts`), which applies the `elasticSource.agentPolicyNamePattern` regex (default `(.*)\s\|\s.*`, see `getAgentPolicyNamePattern` in `src/config.ts`) and, if it matches, uses capture group 1 as the name instead of the full live name - e.g. `Agent Policy1 | Test` downloads as `Agent Policy1`, and `system-datatech-datacentral | test` downloads as `system-datatech-datacentral`, letting the same policy pulled from several environments (`... | Test`, `... | Prod`, `... | Dev`) land under one consistent local name rather than one folder per environment. `downloadIntegrationPolicy` runs its live `name` through the exact same helper (and the same `validateArtifactName` check) as `downloadAgentPolicy` does for its own name - the setting is shared rather than duplicated per artifact type, despite its agent-policy-specific setting name. Neither download's `id`/`policy_id`/`policy_ids` fields are touched by this - only the locally-facing `name` (and therefore the folder/file name) is affected. If the pattern doesn't match, or the setting holds invalid regex syntax, the full original live name is used unchanged rather than failing the download.