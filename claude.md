...
Clicking a live integration policy opens it as a **read-only "virtual document"** rather than a webview panel, deliberately unlike every other live/read-only view in this project: `src/editors/liveIntegrationPolicyDocumentProvider.ts` registers a `vscode.TextDocumentContentProvider` on a custom `elastic-live-integration-policy` scheme (no file on disk, so it's inherently read-only - there's no save flow to hijack), and `elasticSource.openLiveIntegrationPolicy` (`src/extension.ts`) opens it directly via `markdown.showPreview` so it renders as a formatted screen. Its content mirrors the structured input screen the editable `IntegrationPolicyEditorPanel` form uses - one markdown section per input, one subsection per stream, each enabled/disabled state and a table of the template's var labels against the policy's actual values - built from the same `PackageTemplate`/`resolveIntegrationTemplate` (`src/integrations/registry.ts`) the editable form uses. If no structured template is registered for that package/version, it falls back to a raw JSON dump of `inputs`/`vars`, mirroring the editable form's own JSON fallback.
