...
function input(id: string) {
  const found = nginxPackageTemplate_2_3_2.inputs.find((i) => i.id === id);
...
describe('nginxPackageTemplate_2_3_2', () => {
  it('is structurally well-formed', () => {
    assertTemplateIsWellFormed(nginxPackageTemplate_2_3_2);
  });

  it('is package version 2.3.2, matching the published EPR package snapshot', () => {
    expect(nginxPackageTemplate_2_3_2.name).toBe('nginx');
    expect(nginxPackageTemplate_2_3_2.title).toBe('Nginx');
    expect(nginxPackageTemplate_2_3_2.version).toBe('2.3.2');
  });

  it('has the two input types Nginx declares, keyed as <package>-<type>', () => {
    expect(nginxPackageTemplate_2_3_2.inputs.map((i) => i.id).sort()).toEqual([
      'nginx-logfile',
      'nginx-nginx/metrics',
    ]);
  });
  ...
  it('like 2.0.0, and unlike the newer 3.2.1 template, neither input declares a `condition` var', () => {
  ...
  it('no stream declares requiresRoot, so a new Nginx 2.3.2 policy always computes requires_root=false', () => {
    for (const i of nginxPackageTemplate_2_3_2.inputs) {
      for (const s of i.streams) {
        expect(s.requiresRoot).toBeFalsy();
      }
    }
    const inputs = buildDefaultInputs(nginxPackageTemplate_2_3_2);
    expect(computeRequiresRoot(nginxPackageTemplate_2_3_2, inputs)).toBe(false);
  });
