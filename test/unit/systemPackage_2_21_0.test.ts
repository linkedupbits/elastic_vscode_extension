...
function input(id: string) {
  const found = systemPackageTemplate_2_21_0.inputs.find((i) => i.id === id);
...
describe('systemPackageTemplate_2_21_0', () => {
  it('is structurally well-formed', () => {
    assertTemplateIsWellFormed(systemPackageTemplate_2_21_0);
  });

  it('is package version 2.21.0, structurally identical to 2.22.1', () => {
    expect(systemPackageTemplate_2_21_0.name).toBe('system');
    expect(systemPackageTemplate_2_21_0.version).toBe('2.21.0');
  });

  it('has the four input types System declares, keyed as <package>-<type>', () => {
    expect(systemPackageTemplate_2_21_0.inputs.map((i) => i.id).sort()).toEqual([
      'system-journald',
      'system-logfile',
  ...
  it('system.core defaults to disabled, matching manifest enabled:false (was true in older versions)', () => {
  ...
  it('a brand-new System policy computes requires_root=true (auth/syslog enabled by default)', () => {
    const inputs = buildDefaultInputs(systemPackageTemplate_2_21_0);
    expect(computeRequiresRoot(systemPackageTemplate_2_21_0, inputs)).toBe(true);
  });
  ...
  it('requires_root computes false once every root-needing stream is disabled', () => {
    ...
    expect(computeRequiresRoot(systemPackageTemplate_2_21_0, inputs)).toBe(false);
  });
