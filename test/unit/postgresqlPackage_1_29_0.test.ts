...
function input(id: string) {
  const found = postgresqlPackageTemplate_1_29_0.inputs.find((i) => i.id === id);
...
describe('postgresqlPackageTemplate_1_29_0', () => {
  it('is structurally well-formed', () => {
    assertTemplateIsWellFormed(postgresqlPackageTemplate_1_29_0);
  });

  it('is package version 1.29.0, matching the published EPR package snapshot', () => {
    expect(postgresqlPackageTemplate_1_29_0.name).toBe('postgresql');
    expect(postgresqlPackageTemplate_1_29_0.version).toBe('1.29.0');
    expect(postgresqlPackageTemplate_1_29_0.title).toBe('PostgreSQL');
  });

  it('has the two input types PostgreSQL declares, keyed as <package>-<type>', () => {
    expect(postgresqlPackageTemplate_1_29_0.inputs.map((i) => i.id).sort()).toEqual([
      'postgresql-logfile',
  ...
  it('the log stream has a new (since 1.28.0), non-required tz_map var', () => {
    const tzMap = stream('postgresql-logfile', 'postgresql.log').vars.find((f) => f.key === 'tz_map');
    expect(tzMap?.type).toBe('multiline');
    expect(tzMap?.default).toContain('tz_short: AEST');
    expect(tzMap?.required).toBeUndefined();
  });
  ...
  it('no stream declares requiresRoot, so a new PostgreSQL policy always computes requires_root=false', () => {
    for (const i of postgresqlPackageTemplate_1_29_0.inputs) {
      for (const s of i.streams) {
        expect(s.requiresRoot).toBeFalsy();
      }
    }
    const inputs = buildDefaultInputs(postgresqlPackageTemplate_1_29_0);
    expect(computeRequiresRoot(postgresqlPackageTemplate_1_29_0, inputs)).toBe(false);
  });
