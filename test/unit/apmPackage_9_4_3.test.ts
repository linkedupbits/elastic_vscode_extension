...
function input(id: string) {
  const found = apmPackageTemplate_9_4_3.inputs.find((i) => i.id === id);
...
describe('apmPackageTemplate_9_4_3', () => {
  it('is structurally well-formed', () => {
    assertTemplateIsWellFormed(apmPackageTemplate_9_4_3);
  });

  it('is named apm, titled Elastic APM, at version 9.4.3', () => {
    expect(apmPackageTemplate_9_4_3.name).toBe('apm');
    expect(apmPackageTemplate_9_4_3.title).toBe('Elastic APM');
    expect(apmPackageTemplate_9_4_3.version).toBe('9.4.3');
  });

  it('has a single input, keyed apm-<manifestInputType> per the naming convention', () => {
    expect(apmPackageTemplate_9_4_3.inputs.map((i) => i.id)).toEqual(['apm-apm']);
  });
  ...
  it('host/url default to localhost:8200, matching the raw 9.3.5-preview manifest.yml', () => {
  ...
  it('new since 9.0.3: tail_sampling_ttl defaults 30m and tail_sampling_discard_on_write_failure defaults false', () => {
    const vars = input('apm-apm').vars ?? [];
    expect(vars.find((f) => f.key === 'tail_sampling_ttl')).toMatchObject({ type: 'string', default: '30m' });
    expect(vars.find((f) => f.key === 'tail_sampling_discard_on_write_failure')).toMatchObject({
      type: 'boolean',
      default: false,
    });
  });
  ...
  it('no stream declares requiresRoot (manifest has no agent.privileges.root), so requires_root computes false', () => {
    for (const i of apmPackageTemplate_9_4_3.inputs) {
      for (const s of i.streams) {
        expect(s.requiresRoot).toBeFalsy();
      }
    }
    const inputs = buildDefaultInputs(apmPackageTemplate_9_4_3);
    expect(computeRequiresRoot(apmPackageTemplate_9_4_3, inputs)).toBe(false);
  });
});
